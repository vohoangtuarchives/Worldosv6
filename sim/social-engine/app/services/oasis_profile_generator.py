...
from ..utils.reality_loader import reality_loader
from ..utils.worldos_cache import worldos_cache
...
class OasisProfileGenerator:
    ...
    def generate_profile_from_entity(
        ...
        user_id: int,
        use_llm: bool = True,
        world_context_instruction: str = "",
        era_id: Optional[str] = None,
        power_id: Optional[str] = None
    ) -> OasisAgentProfile:
        """
        Từ Zep Entity sinh ra OASIS Agent Profile phù hợp bối cảnh thực tại.
        """
        entity_type = entity.get_entity_type() or "Entity"
        
        # Bổ sung bối cảnh thực tại từ Knowledge Base
        deep_context = ""
        if era_id:
            deep_context += reality_loader.get_era_context(era_id)
        if power_id and era_id:
            deep_context += "\n" + reality_loader.get_power_context(power_id, era_id)
            
        full_world_context = (world_context_instruction + "\n" + deep_context).strip()
        ...
        if use_llm:
            # Sử dụng LLM để sinh nhân phẩm chuyên sâu
            profile_data = self._generate_profile_with_llm(
                ...
                context=context,
                world_context_instruction=full_world_context
            )
        else:
            # 使用规则生成基础人设
    ...
    def _generate_profile_with_llm(
        ...
        # 1. Kiểm tra Cache trước khi gọi LLM
        cache_messages = [
            {"role": "system", "content": self._get_system_prompt(is_individual, world_context_instruction)},
            {"role": "user", "content": prompt}
        ]
        params = {"model": self.model_name, "temperature": 0.7}
        cached_result = worldos_cache.get_response(cache_messages, params)
        if cached_result:
            logger.info(f"OASIS Cache Hit cho thực thể: {entity_name}")
            return cached_result

        # 2. Thực hiện gọi LLM nếu Cache Miss
        max_attempts = 3
        last_error = None
        ...
        for attempt in range(max_attempts):
            try:
                ...
                try:
                    ...
                    worldos_cache.set_response(cache_messages, params, result)
    ...
    def _get_system_prompt(self, is_individual: bool, world_context_instruction: str = "") -> str:
        """Lấy System Prompt tích hợp bối cảnh đa vũ trụ"""
        base_prompt = "Bạn là chuyên gia về tâm lý học và nhân chủng học đa vũ trụ. Nhiệm vụ của bạn là tạo ra nhân sắc (Persona) cực kỳ chi tiết, chân thực cho các Agent trong hệ thống mô phỏng WorldOS. Phải trả về định dạng JSON hợp lệ. Sử dụng Tiếng Việt (trừ khi có yêu cầu khác)."
        
        if world_context_instruction:
            base_prompt += f"\n\n[BỐI CẢNH THỰC TẠI (WORLD REALITY)]: \n{world_context_instruction}\n\nLƯU Ý CỰC KỲ QUAN TRỌNG: Nhân vật PHẢI phù hợp 100% với bối cảnh này. Nếu là thời Tiền sử, nhân vật không được biết về công nghệ hiện đại. Nếu là Đông phương Cổ đại, nhân vật phải dùng thuật ngữ võ hiệp."
    ...
    def _build_individual_persona_prompt(
        ...
        """Xây dựng Prompt sinh nhân sắc cá nhân (Individual Persona)"""
        
        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "Không có"
        context_str = context[:3000] if context else "Không có bối cảnh bổ sung"
        
        return f"""Tạo cấu trúc nhân phẩm (Persona) chi tiết cho thực thể dưới đây dựa trên bối cảnh thực tại đã cung cấp.

Tên thực thể: {entity_name}
Loại thực thể: {entity_type}
Tóm tắt: {entity_summary}
Thuộc tính: {attrs_str}

Bối cảnh quan hệ & Sự kiện:
{context_str}

Hãy tạo JSON bao gồm các trường:

1. bio: Giới thiệu ngắn về nhân vật (phù hợp với bối cảnh).
2. persona: Mô tả nhân sắc chi tiết (khoảng 2000 từ), bao gồm:
   - Thông tin cơ bản (Tuổi, nghề nghiệp thời đại, xuất thân).
   - Tâm lý & Tính cách (Dựa trên MBTI nhưng diễn đạt bằng ngôn ngữ bối cảnh).
   - Hành vi xã hội (Cách tương tác với người khác trong bối cảnh này).
   - Mục tiêu & Lý tưởng (Điều nhân vật khao khát trong thế giới này).
   - Ký ức & Sự kiện (Mối liên hệ của nhân vật với các sự kiện đang diễn ra).
3. age: Tuổi (Số nguyên).
4. gender: Giới tính ("male" hoặc "female").
5. mbti: Loại MBTI.
6. country: Vùng lãnh thổ/Quốc gia (Bằng tiếng Việt).
7. profession: Nghề nghiệp (PHẢI phù hợp với bối cảnh kỷ nguyên).
8. interested_topics: Các chủ đề quan tâm.

QUAN TRỌNG: 
- Tất cả các giá trị phải là chuỗi hoặc số, không dùng ký tự xuống dòng.
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        """Xây dựng Prompt sinh thiết lập tổ chức/nhóm (Group Persona)"""
