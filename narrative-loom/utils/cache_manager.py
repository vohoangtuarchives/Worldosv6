...
class WorldOsCacheManager:
    def __init__(self):
        ...
        try:
            ...
            print("DEBUG: CacheManager - Redis Connected.")
        except Exception as e:
            print(f"WARNING: CacheManager - Redis failing: {e}. Fallback to No-Op/In-Memory.")
    ...
    def get_cached_narrative(self, world_id: int, current_tick: int, prompt: str) -> Optional[str]:
        ...
        try:
            ...
            if (current_tick - tick_produced) > self.ttl_ticks:
                print(f"DEBUG: Cache Expired for world {world_id} (Age: {current_tick - tick_produced} ticks)")
    ...
    def invalidate_world_cache(self, world_id: int):
        ...
        print(f"DEBUG: Cache Invalidated for world {world_id}.")
