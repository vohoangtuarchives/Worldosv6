...
export function SocietyMetricsPanel({ universeId, refreshTrigger = 0 }: { universeId: number | null; refreshTrigger?: number }) {
  ...
  if (loading) return <div className="text-xs text-muted-foreground p-2">Đang tải chỉ số xã hội…</div>;
  ...
  return (
    <div className="space-y-3 text-xs">
      ...
      <div className="flex items-center gap-2 p-2 rounded bg-muted/50 border border-border/50">
        ...
        <span className="font-mono text-foreground">{data.total_population}</span>
      </div>
      {settlements.length > 0 && (
        <div className="p-2 rounded bg-muted/30 border border-border/50">
          <div className="text-[10px] text-muted-foreground mb-1">Khu vực (settlement)</div>
          <div className="space-y-1 max-h-24 overflow-auto">
            {settlements.slice(0, 8).map(([zoneIdx, s]) => (
              <div key={zoneIdx} className="flex justify-between gap-2 text-foreground">
                <span>Z{zoneIdx}: {levelLabels[s.level] ?? s.level} · {govLabels[s.governance] ?? s.governance}</span>
                <span className="font-mono text-muted-foreground">{s.population} dân</span>
              </div>
            ))}
            {settlements.length > 8 && <div className="text-muted-foreground text-[10px]">+{settlements.length - 8} khu</div>}
          </div>
        </div>
      )}
      {data.economy != null && (
        <div className="flex items-center gap-2 p-2 rounded bg-muted/50 border border-border/50">
          ...
          <div>
            <div className="text-muted-foreground">Kinh tế</div>
            <div className="font-mono text-foreground text-[10px]">Thặng dư: {data.economy.total_surplus.toFixed(1)} · Tiêu thụ: {data.economy.total_consumption.toFixed(1)}</div>
          </div>
        </div>
      )}
      {data.politics != null && (
        <div className="flex items-center gap-2 p-2 rounded bg-muted/50 border border-border/50">
          ...
          <div>
            <div className="text-muted-foreground">Chính trị</div>
            <div className="font-mono text-foreground text-[10px]">Ổn định: {(data.politics.stability * 100).toFixed(0)}% · Hợp pháp: {(data.politics.legitimacy * 100).toFixed(0)}%</div>
          </div>
        </div>
      )}
      {data.war != null && (
        <div className="flex items-center gap-2 p-2 rounded bg-muted/50 border border-border/50">
          ...
          <div>
            ...
            <div className="font-mono text-foreground text-[10px]">Sức mạnh: {data.war.military_power.toFixed(2)} · Áp lực: {(data.war.conflict_pressure * 100).toFixed(0)}%</div>
          </div>
        </div>
      )}
      {settlements.length === 0 && !data.economy && !data.politics && !data.war && (
        <div className="text-muted-foreground text-[10px] p-2">
          Chưa có dữ liệu. Dùng nút <strong className="text-muted-foreground">Advance</strong> (1 tick) hoặc <strong className="text-muted-foreground">Pulse</strong> (nhiều tick) trên đầu trang để chạy mô phỏng — engine sẽ cập nhật settlement, kinh tế, chính trị.
        </div>
      )}
    </div>
  );
