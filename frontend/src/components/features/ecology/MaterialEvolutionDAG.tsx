...
const MaterialDataNode = ({ data }: MaterialNodeProps) => (
  <div
    className={`p-3 rounded-lg border-2 w-48 shadow-lg transition-transform hover:scale-105 ${
      data.isActive
        ? "bg-muted border-green-500 shadow-[0_0_15px_rgba(34,197,94,0.3)]"
  ...
  >
    ...
    <div className="flex justify-between items-start mb-2">
      <div className="flex items-center gap-2">
        <Layers className={`w-4 h-4 ${data.isActive ? "text-green-400" : "text-muted-foreground"}`} />
        <span className="font-bold text-foreground text-sm truncate">{data.label}</span>
      </div>
    ...
    <div className="space-y-1 mt-2 border-t border-border/50 pt-2">
      <div className="flex justify-between text-xs font-mono">
        ...
        <span className="text-foreground">I{data.tier === 1 ? "" : data.tier === 2 ? "I" : data.tier === 3 ? "II" : "V"}</span>
      </div>
      <div className="flex justify-between text-xs font-mono">
        <span className="text-muted-foreground">Năng lượng (kV):</span>
        <span className="text-yellow-400">{(data.energy || 0).toFixed(1)}</span>
      </div>
      <div className="text-[9px] text-muted-foreground font-mono mt-1 w-full truncate">{data.signature}</div>
    </div>
...
export const MaterialEvolutionDAG: React.FC<DAGProps> = ({ instances: initialInstances, universeId }) => {
  ...
  return (
    <div className="relative w-full h-[600px] bg-card/80 rounded-xl border border-border p-1">
      <div className="absolute top-4 left-4 flex items-center gap-2 text-muted-foreground z-10 pointer-events-none bg-card/50 px-3 py-2 rounded-lg backdrop-blur-sm border border-border">
        <Activity className="w-5 h-5 text-green-400" />
        <div className="flex flex-col">
          <span className="text-sm font-bold text-foreground">Tiến hóa Material</span>
          <span className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground">Đồ thị có hướng không chu trình</span>
        </div>
      </div>
