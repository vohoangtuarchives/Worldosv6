...
export default function CreateSeriesModal({ universeId, onCreated, onClose }: CreateSeriesModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="relative w-full max-w-md rounded-[var(--radius)] border border-border bg-card shadow-2xl p-6">
        ...
        <h2 className="text-lg font-semibold text-gradient-cosmos mb-4">✦ Tạo Bộ Truyện Mới</h2>
        <div className="space-y-4">
          <div>
            <label className="block text-xs text-muted-foreground mb-1">Tên bộ truyện</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-xs text-muted-foreground mb-1">Thể loại (Genre)</label>
            <select
              className="w-full rounded-[var(--radius)] border border-border bg-muted px-3 py-2 text-sm text-foreground outline-none focus:border-[hsl(var(--left-brain))] transition-colors"
              value={genre}
          ...
          {error && <p className="rounded-[var(--radius)] bg-destructive/15 border border-destructive/30 px-3 py-2 text-xs text-destructive">{error}</p>}
        </div>
        <div className="mt-6 flex justify-end gap-3">
          <button onClick={onClose} className="rounded-[var(--radius)] border border-border bg-muted px-4 py-2 text-sm text-muted-foreground hover:bg-card transition-colors">
            Hủy
          </button>
          <button
