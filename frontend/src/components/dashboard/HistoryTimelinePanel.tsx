      ...
      {data.timeline.length === 0 ? (
        ...
        <div className="space-y-2 max-h-48 overflow-auto">
          {Object.entries(data.by_type).map(([type, entries]) => {
            ...
            return (
              <div key={type} className="rounded bg-muted/30 border border-border/50 overflow-hidden">
                <button
                  type="button"
                  onClick={() => toggleType(type)}
                  className="w-full flex items-center gap-1 p-1.5 text-left text-foreground hover:bg-muted/30"
                >
                  {expanded ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
                  <span className="font-medium">{label}</span>
                  <span className="text-muted-foreground font-mono ml-1">({entries.length})</span>
                </button>
                {expanded && (
                  <div className="px-2 pb-2 space-y-1">
                    {entries.slice(0, 5).map((e, i) => (
                      <div key={i} className="text-[10px] text-muted-foreground border-l-2 border-border pl-2">
                        <span className="font-mono text-muted-foreground">Tick {e.from_tick}</span>
                        {e.content && <span className="ml-1 text-foreground truncate block">{e.content.slice(0, 80)}{e.content.length > 80 ? "…" : ""}</span>}
                      </div>
                    ))}
                    {entries.length > 5 && <div className="text-muted-foreground text-[10px]">+{entries.length - 5} sự kiện</div>}
                  </div>
                )}
              </div>
            );
