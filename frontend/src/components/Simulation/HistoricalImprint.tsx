...
export const HistoricalImprint: React.FC<HistoricalImprintProps> = ({ imprints, currentTick }) => {
  return (
    <div className="p-4 bg-slate-900/80 backdrop-blur-md rounded-xl border border-white/10 text-white">
        ...
        {imprints.map((event) => (
          <div key={event.id} className="relative pl-6 border-l-2 border-white/5">
            ...
            <div className="flex justify-between items-start">
              <div>
                <p className="font-medium text-white/90">{event.name}</p>
                <p className="text-xs text-white/40 italic">Tick: {event.tick}</p>
              </div>
      ...
      <div className="mt-6 pt-4 border-t border-white/5 flex justify-between items-center text-[10px] text-white/30 uppercase tracking-widest">
