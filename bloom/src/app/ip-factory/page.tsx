...
import LoomStatusPanel from "@/components/IpFactory/LoomStatusPanel";
...
export default function IpFactoryPage() {
    ...
    useEffect(() => {
        loadSeries();
        api.universes().then((d: { id: number; name?: string }[]) => setUniverses(d)).catch(() => { });
    }, [loadSeries]);
    ...
    const handleGenerate = async () => {
        ...
        setGenError(null);
        try {
            await api.ipFactory.generateChapter(selectedSeries.id, true); // sync for immediate feedback
    ...
    // ── Select first universe for modal ─────────────────────────────────────
                        ...
                        <>
                            ...
                            <div className="flex border-b border-border/60">
                                {(["chapters", "bible"] as const).map((tab) => (
                                    <button
                                        ...
                                        className={`flex-1 py-2.5 text-xs font-semibold transition-colors ${activeTab === tab
                            ...
                            {activeTab === "chapters" && (
                                <div className="p-3 border-b border-border/40">
                                    ...
                                    {genError && (
                                        <p className="mt-1.5 text-[10px] text-destructive">{genError}</p>
                                    )}
                                </div>
                            )}
                                ...
                                {activeTab === "chapters" ? (
                                    loadingChapters ? (
                                        ...
                                        <ChapterList
                                            ...
                                            onSelect={(ch) => setSelectedChapter(ch)}
                                        />
                                    )
                                ) : (
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-center text-muted-foreground gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs mt-1 opacity-70">
                                    Chọn một bộ truyện và sinh chương để bắt đầu hành trình.
                                </p>
                            </div>
                            {!selectedSeries && (
                                <div className="mt-4 flex flex-col items-center gap-3 text-xs">
                                    <div className="flex items-center gap-2 rounded-full border border-border bg-card/30 px-4 py-1.5">
                                        <span className="h-1.5 w-1.5 rounded-full bg-[hsl(var(--left-brain))]" />
                                        Simulation Engine sinh nội dung
                                    </div>
                                    <div className="flex items-center gap-2 rounded-full border border-border bg-card/30 px-4 py-1.5">
                                        <span className="h-1.5 w-1.5 rounded-full bg-[hsl(var(--cosmos))]" />
                                        AI Chronicle viết thành truyện
                                    </div>
                                    <div className="flex items-center gap-2 rounded-full border border-border bg-card/30 px-4 py-1.5">
                                        <span className="h-1.5 w-1.5 rounded-full bg-[hsl(var(--right-brain))]" />
                                        Canonize để ghi nhập lịch sử
                                    </div>
                                </div>
                            )}
                        </div>
