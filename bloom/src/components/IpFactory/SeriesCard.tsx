...
export default function SeriesCard({ series, selected, onClick }: SeriesCardProps) {
    return (
        <button
            onClick={onClick}
            className={`w-full text-left rounded-[var(--radius)] border p-4 transition-all duration-200 ${selected
                    ? "border-[hsl(var(--left-brain)/0.6)] bg-[hsl(var(--left-brain)/0.08)] glow-left-brain"
                    : "border-border bg-card/50 hover:border-[hsl(var(--left-brain)/0.3)] hover:bg-card"
                }`}
        >
            <div className="flex items-start justify-between gap-2">
                ...
                <span
                    className={`shrink-0 rounded-full border px-2 py-0.5 text-xs font-medium capitalize ${STATUS_COLORS[series.status] ?? STATUS_COLORS.draft}`}
                >
                    {series.status}
                </span>
            </div>
