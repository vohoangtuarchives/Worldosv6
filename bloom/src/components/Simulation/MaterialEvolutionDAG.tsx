...
import ReactFlow, { Background, Controls, Node as FlowNode, Edge as FlowEdge, Position, Handle, MarkerType } from 'reactflow';
...
const MaterialDataNode = ({ data }: MaterialNodeProps) => (
    <div className={`p-3 rounded-lg border-2 w-48 shadow-lg transition-transform hover:scale-105 ${data.isActive
        ...
        <div className="space-y-1 mt-2 border-t border-slate-700/50 pt-2">
            ...
            <div className="text-[9px] text-slate-500 font-mono mt-1 w-full truncate">
                {data.signature}
            </div>
        </div>
...
export const MaterialEvolutionDAG: React.FC<DAGProps> = ({ instances }) => {

    // Auto-layout logic (basic hierarchical)
    const { nodes, edges } = useMemo(() => {
        ...
        // Group by tier for simple layouting
                ...
                if (inst.parent_instance_id) {
                    rfEdges.push({
                        ...
                        markerEnd: {
                            ...
                            color: inst.status === 'active' ? '#22c55e' : '#64748b',
                        },
                    });
                }
            });
