...
class TransitionEpochAction
{
     ...
     */
    public function execute(Universe $universe, Epoch $currentEpoch, int $tick, array $nextEpochData): Epoch
    {
        ...
        $world = World::find($universe->world_id);
        if ($world) {
            $axiom = $world->axiom ?? [];
    ...
    protected function generateGrandNarrative(Epoch $old, Epoch $new, Universe $universe): string
    {
