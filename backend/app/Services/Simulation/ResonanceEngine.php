...
class ResonanceEngine
{
    ...
    protected function calculateResonance(Universe $u1, Universe $u2, UniverseSnapshot $s1): float
    {
        ...
        $e2 = $u2->world->epochs()->where('status', 'active')->first(); // Giả định thế giới có thể có nhiều kỷ nguyên song song cho các vũ trụ khác nhau (nếu thiết kế cho phép)
    ...
    protected function updateInteraction(Universe $u1, Universe $u2, float $resonance, int $tick): void
    {
        $interaction = UniverseInteraction::updateOrCreate(
            ...
            [
                ...
                'synchronicity_score' => $resonance * 1.2, // Hệ số khuếch đại đồng bộ
                'payload' => [
                    'last_sync_tick' => $tick,
