...
final class PopulationEngine implements SimulationEngine
{
    ...
    public function runWithState(\App\Simulation\Runtime\State\WorldState $state, int $tick): array
    {
        ...
        if ($result['ok'] ?? false) {
            ...
            // Standardized update
            $state->set('population', $newPop);
            $state->set('last_growth', $growth);
            $state->set('fertility_rate', $fertility);
            $state->set('mortality_rate', $mortality);

            return ['population' => $newPop, 'growth' => $growth];
        }
