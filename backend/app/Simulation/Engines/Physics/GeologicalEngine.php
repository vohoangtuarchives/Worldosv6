 ...
 */
class GeologicalEngine implements \App\Simulation\Contracts\SimulationEngine
{
    ...
    public function handle(\App\Simulation\Runtime\State\WorldState $state, \App\Simulation\Domain\TickContext $ctx): \App\Simulation\Domain\EngineResult
    {
        ...
        if (method_exists($this, 'runWithState')) {
            $effects = $this->runWithState($state, $ctx->getTick());
        ...
        return new \App\Simulation\Domain\EngineResult([], $effects, []);
    }
     ...
     */
    public function runWithState(\App\Simulation\Runtime\State\WorldState $state, int $currentTick): array
    {
        ...
        if ($zonesModified) {
            ...
            return [
                new \App\Simulation\Effects\WorldStateUpdateEffect(['zones' => $zones])
            ];
        }
