...
/**
 * Layer 11: Metabolic Engine 🍎⚡
 * 
 * Manages the energy accounting for the universe.
 * Energy = Growth - Decay - Entropy.
 */
class MetabolicEngine implements SimulationEngine
{
    ...
    public function handle(WorldState $state, TickContext $ctx): EngineResult
    {
        ...
        if (empty($zones)) {
            // Mode cũ: Tính toán toàn cục
            $populations[] = (float) $state->get('fields.population', 0);
            $biomasses[] = (float) $state->get('fields.biomass', $populations[0] * 0.1); 
            $industries[] = (float) $state->get('fields.industrial_activity', 0);
            $isGlobal = true;
        } else {
            // Lùi vòng lặp map qua O(1) FFI call
        ...
        $totalNetEnergy = 0.0;
        if ($isGlobal) {
            $state->set('fields.population', max(0, $populations[0]));
            $state->set('fields.biomass', max(0, $biomasses[0]));
            $totalNetEnergy = $netEnergies[0] ?? 0.0;
        } else {
            foreach ($zones as $i => &$zone) {
                $zone['state']['population'] = max(0, $populations[$i]);
                $zone['state']['biomass'] = max(0, $biomasses[$i]);
        ...
        // Detect Starvation (Global heuristic)
        $avgNetEnergy = $totalNetEnergy / max(1, count($netEnergies));
        if ($avgNetEnergy < -0.5) {
            $state->set('survival_modifier', 0.7); // 30% penalty
