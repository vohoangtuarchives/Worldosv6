...
rule Evaluate_Historical_Trauma
  ...
  category legend
  trigger event_type
  when event_type == "COLLAPSE" or event_type == "GENOCIDE" or event_type == "EXTINCTION"
  then
    calc mythic_weight
      formula "event_intensity * (1.0 + causal_integrity_debt * 0.5)"
    
    when mythic_weight > 0.8
    then
      emit_event CREATE_WORLD_SCAR { 
        type: "TRAUMA", 
        weight: mythic_weight, 
        duration: "PERMANENT" 
      }
...
rule Evaluate_Cultural_Golden_Age
  ...
  category legend
  trigger event_type
  when event_type == "ASCENSION" or event_type == "GREAT_INVENTION" or event_type == "EPOCH_TRANSITION"
  then
    calc heritage_weight
      formula "event_intensity * field_knowledge_field"
    
    when heritage_weight > 0.7
    then
