...
rule Action_Utility_Scoring
  ...
  then
    ...
    # Heroic Acceleration
    when is_heroic then
      when heroic_type == "SCIENTIST" then set score_research (score_research * 2.0)
      when heroic_type == "GENERAL" then set score_battle (score_battle * 2.0)
      when heroic_type == "MERCHANT" then set score_trade (score_trade * 2.0)
      when heroic_type == "PROPHET" then set score_meditate (score_meditate * 2.0)
    end
