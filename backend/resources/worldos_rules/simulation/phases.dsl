...
rule Transition_Genesis_to_Order
  ...
  when epoch == "genesis"
  then
    when field_order > 0.6
    then
      emit_event PHASE_TRANSITION { from: "genesis", to: "ordered_era" }
...
rule Transition_to_Renaissance
  ...
  when 
    ...
    field_belief > 0.5
  then
    emit_event PHASE_TRANSITION { to: "renaissance" }
...
rule Transition_to_Industrial
  ...
  when
    ...
    tech_level > 0.7
  then
    emit_event PHASE_TRANSITION { to: "industrial_era" }
