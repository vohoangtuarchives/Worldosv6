...
rule "detect_eternal_stagnation"
    priority 200
    condition "field_order > 0.8 && innovation_rate < 0.2"
    action {
        drift topology.stagnation_pull target 1.0 speed 0.05
        
        when "topology.stagnation_pull > 0.9" {
...
rule "detect_apotheosis_vortex"
    priority 200
    condition "field_knowledge > 0.9 && field_resonance > 0.7"
    action {
        drift topology.apotheosis_pull target 1.0 speed 0.02
        
        when "topology.apotheosis_pull > 0.95" {
...
rule "detect_heat_death"
    priority 200
    condition "field_entropy > 0.9 && field_meaning < 0.1"
    action {
        drift topology.entropy_vortex_pull target 1.0 speed 0.1
        
        when "topology.entropy_vortex_pull > 0.8" {
...
rule "attractor_gravity_decay"
    ...
    action {
        ...
        when "topology.stagnation_pull < 0.1 && topology.apotheosis_pull < 0.1 && topology.entropy_vortex_pull < 0.1" {
