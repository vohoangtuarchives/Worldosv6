...
rule "heroic_migration_on_collapse"
    condition "active_attractor == 'DARK_AGE' && stability_index < 0.3 && meta.active_causal_bridges.count > 0"
    action {
        ...
        emit_event MULTIVERSE_MIGRATION {
            description: "Các Anh hùng đang vượt qua cửa ngõ nhân quả để tìm kiếm một thực tại ổn định hơn."
        }
    }

rule "resonance_knowledge_leak"
    condition "meta.active_causal_bridges.count > 0"
    action {
...
rule "causal_contamination"
    condition "entropy > 0.8 && meta.active_causal_bridges.count > 0"
    action {
