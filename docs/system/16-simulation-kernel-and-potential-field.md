...
- **Kích hoạt**: Khi `config('worldos.simulation_kernel_post_tick') === true`, sau mỗi tick từ Rust engine, `AdvanceSimulationAction` gọi `SimulationKernel::runTick()` rồi ghi đè snapshot bằng output của kernel.
- **Luồng**: Load `WorldState` từ snapshot → chạy lần lượt các engine đã đăng ký → mỗi engine chỉ **đọc** state và **emit** `Effect[]` → `EffectResolver` áp dụng toàn bộ effect lên bản copy mutable → trả về `WorldState` mới.
...
- `simulation_kernel_post_tick`: bật kernel sau mỗi tick.
