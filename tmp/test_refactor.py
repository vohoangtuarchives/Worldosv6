...
def refactor_dsl_safe(content):
    ...
    for line in lines:
        ...
        if first_word == 'rule':
            current_phase = 'rule'
            name = stripped[4:].strip().rstrip('{').rstrip(':').rstrip(',').strip().strip('"')
        ...
        if first_word == 'when':
            ...
            rest = stripped[4:].strip().rstrip('{').rstrip(':').rstrip(',').strip().strip('"').strip("'")
            if rest:
        ...
        if first_word == 'then':
            ...
            rest = stripped[4:].strip().rstrip('{').rstrip(':').rstrip(',').strip().strip('"').strip("'")
            if rest:
                # If rest starts with an action keyword, it's an action line
                new_lines.append(rest)
            continue
        ...
        if current_phase == 'then':
            ...
            # Lines before any block or outside blocks
            if cleaned:
                new_lines.append(cleaned)
...
if __name__ == "__main__":
    # Test on biosphere.dsl
    path = r"C:\projects\IPFactory\backend\resources\worldos_rules\biology\biosphere.dsl"
    ...
    with open(path + ".tmp", 'w', encoding='utf-8', newline='\n') as f:
        f.write(new_content)
    print("Refactored to biosphere.dsl.tmp")
